use serde::{Deserialize, Serialize};

/// Protocol type for A2A Protocol communication.
#[derive(Serialize, Deserialize, Clone, Copy, Debug, PartialEq, Eq, Hash, Default)]
#[serde(rename_all = "snake_case")]
pub enum ProtocolType {
    /// JSON-RPC protocol.
    #[default]
    JsonRpc,
    /// Model Context Protocol (MCP).
    Mcp,
    /// REST protocol.
    Rest,
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_all_valid_values() {
        let values = vec![
            ("json_rpc", ProtocolType::JsonRpc),
            ("mcp", ProtocolType::Mcp),
            ("rest", ProtocolType::Rest),
        ];

        for (json_str, expected) in values {
            let result: ProtocolType = serde_json::from_str(&format!("\"{}\"", json_str)).unwrap();
            assert_eq!(result, expected, "Failed for value: {}", json_str);
        }
    }

    #[test]
    fn test_invalid_value_rejected() {
        let json = "\"nonexistent_value\"";
        let result: Result<ProtocolType, _> = serde_json::from_str(json);
        assert!(result.is_err(), "Invalid value should be rejected");
    }

    #[test]
    fn test_serialization_roundtrip() {
        let values = vec![ProtocolType::JsonRpc, ProtocolType::Mcp, ProtocolType::Rest];

        for original in values {
            let json = serde_json::to_string(&original).unwrap();
            let deserialized: ProtocolType = serde_json::from_str(&json).unwrap();
            assert_eq!(
                original, deserialized,
                "Serialization roundtrip failed for {:?}",
                original
            );
        }
    }

    #[test]
    fn test_default_value() {
        let default = ProtocolType::default();
        assert_eq!(default, ProtocolType::JsonRpc, "Default should be JsonRpc");
    }
}
